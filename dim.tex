...
\begin{proof}
	Sia $\langle G,s,t\rangle$ la codifica binaria di un grafo orientato $G$ con $n$ nodi e di due vertici distinguibili $s,t$ (i vertici sono numerati $1,\dots,n$ e codificati in binario).
	Costruiamo una macchina di Turing nondeterministica $H$ a tre nastri che decide $HAMPATH$ in tempo polinomiale.

	\textbf{Struttura dei nastri.}
	\begin{itemize}
		\item Nastro 1: contiene l'input $\langle G,s,t\rangle$ (codifica del grafo, ad es.\ matrice di adiacenza o lista di archi).
		\item Nastro 2: viene usato per scrivere la sequenza nondeterministicamente scelta $p_1,\dots,p_n$.
		\item Nastro 3: nastro di lavoro per i controlli (marcature, confronti, ecc.).
	\end{itemize}

	\textbf{Descrizione di $H$ su input $\langle G,s,t\rangle$.}
	\begin{enumerate}
		\item \emph{Generazione nondeterministica.} Nondeterministicamente $H$ scrive sul nastro 2 una sequenza di $n$ numeri $p_1,\dots,p_n$, ciascuno in $\{1,\dots,n\}$. (Ogni possibile sequenza corrisponde a un ramo di computazione.)
		\item \emph{Verifica forma e range.} Se la sequenza non ha esattamente $n$ elementi oppure contiene un elemento fuori dall'intervallo $1,\dots,n$, allora \emph{questo ramo} viene rifiutato.
		\item \emph{Controllo di ripetizioni.} Usando il nastro 3, $H$ verifica che tutti i $p_i$ siano distinti (es.: per ogni $p_i$ controlla se è già stato segnato sul nastro 3; in caso contrario lo marca). Se trova una ripetizione, \emph{questo ramo} viene rifiutato.
		\item \emph{Controllo degli estremi.} verifica che $p_1 = s$ e $p_n = t$. Se uno dei due controlli fallisce, \emph{questo ramo} viene rifiutato.
		\item \emph{Controllo degli archi.} Per ogni $i = 1,\dots,n-1$ verifica che $(p_i,p_{i+1})$ sia un arco di $G$ (cercando nella rappresentazione di $G$ sul nastro 1). Se per qualche $i$ l'arco manca, \emph{questo ramo} viene rifiutato.
		\item \emph{Accettazione.} Se tutti i controlli precedenti passano, allora \emph{questo ramo} accetta l'input.
	\end{enumerate}

	\textbf{Correttezza.} Se esiste un cammino hamiltoniano di $G$ che va da $s$ a $t$, allora esiste almeno una sequenza $p_1,\dots,p_n$ che supera tutti i controlli; la macchina $H$ in corrispondenza della scelta nondeterministica di tale sequenza ha un ramo che accetta, quindi $H$ accetta $\langle G,s,t\rangle$. Viceversa, se $H$ accetta, allora l'esistenza di un ramo accettante fornisce una sequenza che è un cammino hamiltoniano da $s$ a $t$.

	\textbf{Tempo.} La generazione scrive $n$ numeri (ogni numero ha al più $\lceil\log n\rceil$ bit), quindi la scrittura è di lunghezza polinomiale. I controlli (verifica lunghezza e range, controllo ripetizioni, controllo estremi, verifica degli archi) possono essere eseguiti in tempo polinomiale (ad esempio $O(n^2)$ se si usa una ricerca lineare per alcuni passi, oppure $O(n^2)$ anche con matrice di adiacenza), dunque ogni ramo termina in tempo polinomiale. Perciò $H$ è una MdT nondeterministica che decide $HAMPATH$ in tempo nondeterministicamente polinomiale.
\end{proof}
