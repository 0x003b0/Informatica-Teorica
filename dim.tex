...
\footnotesize % riduce la dimensione del font
Indico indecidibile il problema dell'arresto, mentre semidecidibile il linguaggio $\mathcal{L}_{\text{Halt}}$. Ma se $\mathcal{L}_{\text{Halt}}$ 
è il linguaggio associato al problema perchè non posso dire che anche il problema dell'arresto è semidecidibile? \\ \\
Il problema dell'arresto chiede esplicitamente se esiste una MdT $H$ (con input ($R(M),w$)) che \underline{termina sempre}, 
qualunque sia il comportamento di $M$:
\begin{itemize}
  \item Se $M$ termina su $w$, $H$ termina rispondendo "Sì"
  \item Se $M$ non termina su $w$, $H$ termina rispondendo "No"
\end{itemize}
È una richiesta forte: $H$ deve terminare dando una risposta sia quando $M$ termina su $w$ sia quando $M$ non termina su $w$. \\ 
Esiste questa MdT $H$? No. \\ \\
Il linguaggio, invece, fa rifermento solo ad una parte del problema dell'arresto. Ovvero, richiede se $\exists$ una MdT $N$ che 
termina quando $M$ termina su $w$, ma non fa nessun riferimento per quanto riguarda a cosa succede quando $M$ non termina su $w$
(ciò che invece viene richiesto nel problema dell'arresto). \\
È una richiesta più debole: $N$ deve terminare solo quando $M$ termina su $w$. \\
Esiste questa MdT $N$? Sì.
\end{osservazioni}
\begin{theorem}{Halting Problem} 
  IIl problema dell'arresto è indecidibile.
  \footnotesize % riduce la dimensione del font
  \begin{proof}
    Suppongo per assurdo che il problema dell'arresto sia decidibile (ipotesi per assurdo).
    ...
    \begin{itemize}
      ...
      \item Se $M$ non termina su $w$, allora $H'$ rifiuta
    \end{itemize}
    Adesso costruisco un'altra MdT $D$ (che prende in input solo la codifica di una macchina)\footnote{invece dell'input ($R(M),w$), prende in input ($R(M)$)} 
  ...
  E adesso cosa succederebbe se fornissi alla MdT $D$ la propria codifica come input? Otterrei:
    ...
    che il problema dell'arresto sia decidibile è falsa. Pertanto, il problema dell'arresto è indecidibile.
  \end{proof}
\end{theorem}
