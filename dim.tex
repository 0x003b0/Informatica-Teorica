...
\footnotesize % riduce la dimensione del font
  \begin{ragionamento}
    ...
    \begin{enumerate}
      \item La definizione riduzione (o semplicememente la funzione?) \\
  ...
  \begin{proof}
    ...
    dato in input $(R(M),w)$\footnote{istanza del problema dell'arresto}, costruisco una nuova MdT $M'$ che:
    \begin{enumerate}
      \item $M'$ viene avviata (o parte?) su nastro vuoto
      \item $M'$ scrive $w$ sul nastro
      \item $M'$ riporta la testina all'inzio del nastro
      \item $M'$ esegue $M$ su $w$
    \end{enumerate}
    Dato che ho costruito questa funzione di riduzione allora vuol dire che $\mathcal{L}_{\text{Halt}} \leq_m \mathcal{L}_{\text{Blank-Tape}}$ (non posso scrivere cosi forse...)
    e quindi per definizione di riduzione (o meglio scrivere per definizione di funzione di riduzione), 
    $(R(M),w) \in \mathcal{L}_{\text{Halt}} \iff R(M') \in \mathcal{L}_{\text{Blank-Tape}}$ (R(M')? dove l'hai trovato?).
    Quindi, per il teorema, essendo $\mathcal{L}_{\text{Halt}} \leq_m \mathcal{L}_{\text{Blank-Tape}}$ e $\mathcal{L}_{\text{Halt}}$ indecidibile 
    $\implies \mathcal{L}_{\text{Blank-Tape}}$ è indecidibile. 
